...
public class Receive1 {
    public static void main(String[] args) {
        ...
        try {
            ...
            channel.queueDeclare("topicQ1",true,false,false,null);
            channel.exchangeDeclare("shy1","topic",true);
            channel.queueBind("topicQ1","shy1","aa.*");

            channel.basicConsume("topicQ1",true,"",new DefaultConsumer(channel){
                public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                    String message = new String(body);
                    System.out.println("消费者qaa.*----"+message);
                }
            });
        } catch (IOException e) {
            e.printStackTrace();
